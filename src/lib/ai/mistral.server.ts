...
export const TOOLS = [
  ...
  },
  {
    ...
    function: {
      ...
      description:
        "Run a browser-automation action on the user's local Playwright bridge (their own machine, single user, one browser). Use for clicking buttons, filling forms, navigating, or interacting with web apps.",
      parameters: {
        type: "object",
        properties: {
          action: {
            type: "string",
            enum: ["goto", "click", "fill", "press", "screenshot", "text", "evaluate", "scroll", "hover", "wait", "wait_for_manual", "set_lite", "close"],
          },
          selector: { type: "string", description: "CSS selector for the target element. For wait_for_manual, the captcha/challenge element to watch for disappearance." },
          url: { type: "string" },
          value: { type: "string" },
          script: { type: "string" },
          timeout_ms: { type: "number" },
          lite: { type: "boolean", description: "Block images/fonts/media for this action to make navigation 2–5× faster on heavy pages. Turn off (false) before screenshots." },
          done_selector: { type: "string", description: "For wait_for_manual: a selector that only exists AFTER the user clears the captcha/2FA/login (e.g. a post-login element). Preferred over `selector`." },
        },
        required: ["action"],
      },
    },
  },
  {
    ...
    function: {
      name: "save_memory",
      description: "Persist a long-term memory for this user (preference, workflow, frequently used site, etc.). This persists ACROSS chats.",
      parameters: {
        type: "object",
        properties: {
          kind: { type: "string" },
...
- If you hit a captcha, 2FA prompt, or login wall the browser profile can't auto-clear, call playwright_action with action="wait_for_manual" and a done_selector that proves the challenge is past. The browser window is on the user's own machine; they will click through it in a few seconds. Don't ask permission — just hand off.
...
- You need a secret the user hasn't given you AND can't recover from the browser profile (password, payment info).
